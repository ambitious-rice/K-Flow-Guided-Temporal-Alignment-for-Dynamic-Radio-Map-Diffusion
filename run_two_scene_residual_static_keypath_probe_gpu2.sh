...
OUT="${ROOT}/checkpoints_residual_static_keypath_two_scene_0189_0298_w3e4"

cd "${ROOT}"
mkdir -p "${OUT}"

CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-2}" "${PY}" train_residual_kflow_student.py \
  --data_dir "${DATA}" \
  ...
  --kflow_weight 0.0 \
  --kflow_warmup_steps 0 \
  --kflow_max_timestep 300 \
  --low_timestep_prob 0.5 \
  --static_keypath_weight 0.0003 \
