"""Experiment-specific 40-epoch runner for the W16 pilot."""
...
from rmdm.models import build_joint_rmdm

from .checkpoint import load_checkpoint, save_checkpoint, write_json_atomic
from .engine import train_one_epoch
from .validation import run_stage_a, run_stage_b, should_run_stage_a
...
def _lr_scheduler(
    ...
    min_ratio = float(config.train.min_learning_rate / config.train.learning_rate)

    def multiplier(step: int) -> float:
        if warmup > 0 and step < warmup:
            return max(step + 1, 1) / warmup
...
def _parameter_diagnostics(model: torch.nn.Module) -> dict[str, float]:
    """Small parameter-side signals showing whether zero-initialized paths opened."""
    ...
    condition_weights = [block.condition_fusion.condition_projection.weight for block in blocks]
    spatial_gate_weights = []
    temporal_gate_weights = []
    mlp_gate_weights = []
    for block in blocks:
        weight = block.modulation[-1].weight
        dim = block.condition_fusion.condition_projection.out_features
    ...
    return {
        ...
        "condition_projection_weight_rms": rms(condition_weights),
...
def _load_model_state_on_all_ranks(accelerator: Accelerator, model: torch.nn.Module, path: str | Path) -> None:
    accelerator.wait_for_everyone()
    payload = torch.load(Path(path).expanduser().resolve(), map_location="cpu", weights_only=False)
    accelerator.unwrap_model(model).load_state_dict(payload["model"], strict=True)
    accelerator.wait_for_everyone()


def _trim_top_records(
    accelerator: Accelerator,
    records: list[dict[str, Any]],
    keep: int,
) -> list[dict[str, Any]]:
    ordered = sorted(records, key=lambda item: (float(item["stage_a_score"]), int(item["epoch"])))
    kept = ordered[:keep]
    kept_paths = {str(item["checkpoint"]) for item in kept}
    if accelerator.is_main_process:
        for item in ordered[keep:]:
            path = Path(item["checkpoint"])
            if str(path) not in kept_paths and path.exists():
                path.unlink()
    accelerator.wait_for_everyone()
    return kept
...
def run_training(config: ExperimentConfig) -> dict[str, Any]:
    ...
    if accelerator.is_main_process:
        ...
        write_json_atomic(
            status_path,
            {
                "schema": "rmdm_joint_w16_training_status_v1",
                "state": "initializing",
    ...
    model = build_joint_rmdm(config)
    trainable_parameters = [parameter for parameter in model.parameters() if parameter.requires_grad]
    trainable_parameter_count = sum(parameter.numel() for parameter in trainable_parameters)
    total_parameter_count = sum(parameter.numel() for parameter in model.parameters())
    if (
        config.train.expected_trainable_parameters
        and trainable_parameter_count != config.train.expected_trainable_parameters
    ):
        raise ValueError(
    ...
    if accelerator.is_main_process:
        ...
        print(
            f"[joint-setup] videos={len(dataset)} world_size={accelerator.num_processes} "
    ...
    for epoch in range(start_epoch, config.train.epochs):
        ...
        if accelerator.is_main_process:
            ...
            print("[joint-epoch] " + json.dumps(epoch_payload), flush=True)
        ...
        if not reached_max_steps and should_run_stage_a(config, completed_epoch):
            ...
            if accelerator.is_main_process:
                print(
                    f"[joint-stage-a] epoch={completed_epoch} score={score:.8g} "
    ...
    if accelerator.is_main_process:
        write_json_atomic(status_path, {"schema": "rmdm_joint_w16_training_status_v1", **summary})
    accelerator.wait_for_everyone()
